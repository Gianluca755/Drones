...
The drones have cellular connectivity and know a few other drones. They are equipped with GPS and we assume they have an ID, otherwise it could be given by the manager server (see below).
...
\begin{itemize}
\item Client: the client sends a request to the broker server for a delivery; % and receives an ack message from it when the request is completed;
...
\item Broker server: this server receives the requests from clients; %and sends back an ack message to the client, it queues requests if there are no drones available at the moment;
