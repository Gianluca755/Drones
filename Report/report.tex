...
\begin{itemize}
\item \textbf{request for a delivery}: a client can request the delivery of a package form point A to point B, the input is the package starting position and the destination, there is no output and the effect is the election of a drone;
\item \textbf{request for the status of a delivery}:the client can request the status of it's request to the broker server , the input is , the output is a message to the client signalling the status of the delivery;
\item \textbf{message after crash while idle}:a drone that crashes while idle can request to be taken to a repair station, the input is the broken drone position, the effect is the election of a drone that will move it to a repair station;
\item \textbf{message after crash after being chosen}:a drone that crashes after it has been chosen can request to be taken to a repair station, the input is the broken drone position and whether it already picked up the package or not , the effect is the election of a drone that will move it to a repair station and the election of another drone to carry out the delivery;
\item \textbf{a new drones to joins the network}:a new drone that wants to join the network sends a message to the management server, the input is the new drone unique id if he has one, its carry weight, the output  from the manager server is the unique id(if needed) and the other drones to connect to, the effect is the entrance of the new drone in the network.
\end{itemize}
