...
\begin{itemize}
\item Client: it's the client who sends a request to the broker server for a delivery;
\item Drone: the drones are the entities that move the packages between locations, new drones that want to join the network send a message to the Fleet management server;
\item Broker server: this server recieves the requests from the clients and sends back an ack message to the client ;
\item Fleet management server: this server handles the entry of a new drone in the network;
