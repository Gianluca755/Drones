...
\begin{itemize}
	...
	\item failure transparency: if a drone crashes the delivery must be completed smoothly for the client, if the broker server or the manager server crash the systems will connect to a secondary backup server;
...
\begin{itemize}
\item request for a delivery: the input is the package starting position and the destination, the effect is the election of a drone;
\item request for the status of a delivery: the input is , the output is a message to the client signalling the status of the delivery;
\item message after crash while idle: the input is the broken drone position, the effect is the election of a drone that will move it to a repair station;
\item message after crash before/after pickup: the input is the broken drone position, the effect is the election of a drone that will move it to a repair station and the election of another drone to carry out the delivery;
\item a new drones to joins the network: the input provided by the new drone is its unique id if he has one, its carry weight, the output  from the manager server is the unique id(if needed) and the other drones to connect to, the effect is the entrance of the new drone in the network.
\end{itemize}
...
\item it's important for following transparencies to be present:
	...
	\item location: the position of the drones is unknown to the clients.
	\end{itemize}
\end{itemize}
